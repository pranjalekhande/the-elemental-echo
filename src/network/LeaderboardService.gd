...
func _is_host() -> bool:
    # For solo play (no multiplayer peer), always act as host for leaderboard
    if not Engine.has_singleton("NetworkManager"):
        return true  # If no NetworkManager, assume solo mode
    
    var nm = Engine.get_singleton("NetworkManager")
    
    # If no multiplayer peer is active, we're in solo mode - act as host
    if not nm.has_method("get") or nm.multiplayer.multiplayer_peer == null:
        return true
    
    # In multiplayer mode, check if we're the actual host
    return nm.is_host  # Direct property access instead of nm.get("is_host") 
